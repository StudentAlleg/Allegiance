...
void CDxtexView::Zoom(float zoomfactor)
{
    ...
    //scroll so that the center position remains
    // calculate the new upper-left position
    ptCenter.x = (LONG)(ptCenter.x * zoomfactor);
    ptCenter.y = (LONG)(ptCenter.y * zoomfactor);
    ptUpperLeft.x = ptCenter.x - windowRect.right/2;
    ptUpperLeft.y = ptCenter.y - windowRect.bottom/2;
