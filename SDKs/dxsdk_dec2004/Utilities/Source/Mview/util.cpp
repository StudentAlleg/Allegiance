    ...
    )
{
    ...
    *pSkinningTypesSupported = 0;
    ...
    for (iTech = 0; iTech < EffectDesc.Techniques; iTech++)
    {
        ...
        {
            StringCchPrintfA(szBuf, 256, "%s_BlendWeights", TechDesc.Name);
            ...
            StringCchPrintfA(szBuf, 256, "%s_BlendPaletteSize", TechDesc.Name);
            hr = pEffect->GetInt(szBuf, &iTemp);
...
const TCHAR* DXUtil_GetDXSDKMediaPath()
{
    ...
    StringCchCat( strPath, MAX_PATH, _T("\\Media\\") );
...
//-----------------------------------------------------------------------------
HRESULT DXUtil_FindMediaFile( TCHAR* strPath, UINT strPathLen, TCHAR* strFilename )
{
    HANDLE file;
    ...
    {
        StringCchCopy( strPath, strPathLen, strFullPath );
        CloseHandle( file );
        return S_OK;
    }
    ...
    if( INVALID_HANDLE_VALUE != file )
    {
        StringCchCopy( strPath, strPathLen, strShortName );
        CloseHandle( file );
        return S_OK;
    }
    
    // last, check if the file exists in the media directory
    StringCchPrintf( strPath, strPathLen, _T("%s%s"), DXUtil_GetDXSDKMediaPath(), strShortName );
    ...
    StringCchCopy( strPath, strPathLen, strFilename );
    return E_FAIL;
}
    ...
    )
{
            ...
            case D3DXEDT_STRING:
                ...
                if (FAILED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        ...
                        {
                            char szBuf[1024];
                            StringCchPrintfA(szBuf, 1024, "%s%s", pDefault->pParamName, "@Name");
    ...
    for(UINT iParam = 0; iParam < EffectDesc.Parameters; iParam++)
    {
        ...
        if (pstrName != NULL)
        {
            ...
            DXUtil_FindMediaFile( strTexturePath, 512, (TCHAR*)pstrName );
