...
TrivialData::TrivialData( )
{
    ...
    m_fExtrusion = 0.1f;
    m_fDeviation = 0.001f;
    StringCchCopyA(m_szText, 256, "Hello");
    ...
    if(lpCmdLine)
    {
        ...
        for(;;)
        {
            ...
            else
            {
                ...
                for(ich=0; (pchMin < pchLim) && (ich < (sizeof(szFilenameBuffer)-1)); pchMin++,ich++)
    ...
    // indent so that App will get destroyed before memory check
    {
        ...
        if (bFilenameProvided)
        {
                ...
                !(szFilename[0] == '\\' && szFilename[1] == '\\') )
            {
			    ...
			    StringCchCatA(App.m_szInitialFilename, 256, "\\");
			    StringCchCatA(App.m_szInitialFilename, 256, szFilename);
            }
            else
            {
                StringCchCopyA(App.m_szInitialFilename, 256, szFilename);
            }
...
TrivialData::UpdateSelectionInfo()
{
    ...
    if (m_bVertexSelectionMode)
    {
        if (m_dwVertexSelected != UNUSED32)
        {
            ...
            {
                StringCchPrintfA(szBuf, 80, "Vertex %d", m_dwVertexSelected);
            }
            else
            {
                StringCchPrintfA(szBuf, 80, "Vertex %d", m_dwVertexSelected);
        ...
        {
            StringCchPrintfA(szBuf, 80, "No vertex selected");
        }
    }
    else if (m_bFaceSelectionMode)
    {
        if (m_dwFaceSelected != UNUSED32)
        {
            if (m_pmcSelectedMesh->ptmDrawMesh != NULL)
            {
                ...
                for (iPoint = 0; iPoint < 3; iPoint++)
                {
                    ...
                    if (m_pmcSelectedMesh->rgdwAdjacency != NULL)
                    {
                        if (m_pmcSelectedMesh->rgdwAdjacency[m_dwFaceSelected * 3 + iPoint] == UNUSED32)
                            StringCchPrintfA(rgszAdj[iPoint], 30, "UNUSED");
                        else
                            StringCchPrintfA(rgszAdj[iPoint], 30, "%d", m_pmcSelectedMesh->rgdwAdjacency[m_dwFaceSelected * 3 + iPoint]);
                    }
                    else
                    {
                        StringCchPrintfA(rgszAdj[iPoint], 30, "UNUSED");
                ...
                StringCchPrintfA(szBuf, 80, "Face %d  Ind (%d, %d, %d) Adj (%s, %s, %s)\n", m_dwFaceSelected, 
                                            rgdwFace[0], rgdwFace[1], rgdwFace[2],
                                             rgszAdj[0],  rgszAdj[1],  rgszAdj[2]);
            }
            else
            {
                StringCchPrintfA(szBuf, 80, "Face %d\n", m_dwFaceSelected);
            }
        }
        else
        {
            StringCchPrintfA(szBuf, 80, "No face selected");
        }
    }
    else
    {
