...
DPF_HR(HRESULT hr, const char *szFmt, ...)
{
    ...
    va_list va;
    va_start(va, szFmt);
    StringCbVPrintfA(szMsg, sizeof(szMsg), szFmt, va);
    va_end(va);

    szHr = DXGetErrorString8(hr);

    StringCbPrintfA(sz, sizeof(sz), "d3dxapp: %s (%s)\r\n", szMsg, szHr);
    sz[sizeof(sz)-1] = '\0';
    OutputDebugString(sz);
}
#else
...
CD3DXApplication::Reset(D3DPRESENT_PARAMETERS *pPresentParameters)
{
    ...
    if(EXISTING != Action)
    {
        ...
        switch(Action)
        {
        case RESET:  StringCchCopyA(szDbg, 256, "reset "); break;
        case CREATE: StringCchCopyA(szDbg, 256, "create "); break;
        default:     StringCchCopyA(szDbg, 256, "??? "); break;
        }

        StringCchPrintfA(sz,256, "(card%d ", Device.m_uAdapter);
        StringCchCatA(szDbg,256, sz);

        if(Device.m_dwBehavior & D3DCREATE_PUREDEVICE)
            StringCchCatA(szDbg, 256, "pure ");

        if(Device.m_dwBehavior & D3DCREATE_SOFTWARE_VERTEXPROCESSING)
            StringCchCatA(szDbg, 256, "sw ");
        else if(Device.m_dwBehavior & D3DCREATE_HARDWARE_VERTEXPROCESSING)
            StringCchCatA(szDbg, 256, "hw ");
        else if(Device.m_dwBehavior & D3DCREATE_MIXED_VERTEXPROCESSING)
            StringCchCatA(szDbg, 256, "mixed ");

        if(D3DDEVTYPE_HAL == Device.m_DevType)
            StringCchCatA(szDbg, 256, "hal ");
        else if(D3DDEVTYPE_SW == Device.m_DevType)
            StringCchCatA(szDbg, 256, "emulation ");
        else if(D3DDEVTYPE_REF == Device.m_DevType)
            StringCchCatA(szDbg, 256, "ref ");

        StringCchPrintfA(sz, 256, "%dx%d) ", Params.BackBufferWidth, Params.BackBufferHeight);
        StringCchCatA(szDbg, 256, sz);
