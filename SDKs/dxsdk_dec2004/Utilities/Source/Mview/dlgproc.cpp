...
DlgProcSimplify(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        StringCchPrintfA(szBuf, 256, "%d", cVertices);
...
DlgProcSimplifyFaces(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        StringCchPrintfA(szBuf, 256, "%d", cFaces);
...
LRESULT CALLBACK DlgProcAbout(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            StringCchPrintf( tszVersion, 256, tsz, HIWORD( dwVersionMS ), LOWORD( dwVersionMS ),
    ...
    char **pszOutput)
{
	...
	UINT szOutputLen= 120 * 30;/*vertex info*/
    szOutput = new char[szOutputLen];
    if (szOutput == NULL)
    {
        hr = E_OUTOFMEMORY;
        goto e_Exit;
    ...
    StringCchPrintfA(szOutput, szOutputLen, "Information for Vertex %d:\r\n", dwVertex);
    ...
    while (pDeclCur->Stream != 0xff)
    {
        ...
        StringCchPrintfA(szBuf, 120, "\t%s%d (", szUsage, pDeclCur->UsageIndex);
        StringCchCatA(szOutput,szOutputLen, szBuf);
        ...
        {
            if (iValue == (cValues-1))
                StringCchPrintfA(szBuf,120, "%f", rgfValues[iValue]);
            else
                StringCchPrintfA(szBuf,120, "%f,", rgfValues[iValue]);
        ...
        StringCchCatA(szOutput, szOutputLen,")\r\n");
...
DlgProcVertexInfo(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_INITDIALOG:
        StringCchPrintfA(szBuf,120, "%d", g_pData->m_dwVertexSelected);
    ...
    case WM_COMMAND:
        ...
        case IDC_GETVERTEXINFO:
            ...
            else
            {
                ...
                StringCchPrintfA(szOutput,120, "Index %d is not a valid index.  There are %d vertices in the selected mesh", dwVertex, g_pData->m_pmcSelectedMesh->ptmDrawMesh->GetNumVertices());
            }
    ...
    UINT_IDX Dummy)
{
	...
	UINT szOutputLen= 120 * (10 /*face info*/+ 60/*vertex info*/);
    szOutput = new char[szOutputLen];
    if (szOutput == NULL)
    {
        hr = E_OUTOFMEMORY;
    ...
    StringCchPrintfA(szOutput, szOutputLen, "Information for Face %d:\r\n\r\n", dwFace);

    StringCchPrintfA(szBuf,120, "Vertex Indices:\r\n");
    StringCchCatA(szOutput, szOutputLen, szBuf);
    for (iIndex = 0; iIndex < 3; iIndex++)
    {
        StringCchPrintfA(szBuf,120, "Index %d: %d\r\n", iIndex, pwFaces[dwFace*3 + iIndex]);
        StringCchCatA(szOutput, szOutputLen, szBuf);
    }

    StringCchPrintfA(szBuf, 120, "\r\nAdjacent Face Indices:\r\n");
    StringCchCatA(szOutput, szOutputLen, szBuf);
    for (iIndex = 0; iIndex < 3; iIndex++)
    {
        if (rgdwAdjacency[dwFace*3 + iIndex] != UNUSED32)
            StringCchPrintfA(szBuf, 120, "AdjIndex %d: %d\r\n", iIndex, rgdwAdjacency[dwFace*3 + iIndex]);
        else
            StringCchPrintfA(szBuf, 120, "AdjIndex %d: UNUSED\r\n", iIndex);
    ...
    for (iIndex = 0; iIndex < 3; iIndex++)
    {
        ...
        StringCchCatA(szOutput, szOutputLen, szVertexInfo);
        StringCchCatA(szOutput, szOutputLen, "\r\n");
        delete []szVertexInfo;
    }
...
DlgProcFaceInfo(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    {
    case WM_INITDIALOG:
        StringCchPrintfA(szBuf, 120, "%d", g_pData->m_dwFaceSelected);
    ...
    case WM_COMMAND:
        ...
        case IDC_GETFACEINFO:
            ...
            else
            {
                ...
                StringCchPrintfA(szOutput, 120, "Index %d is not a valid index.  There are %d faces in the selected mesh", dwFace, g_pData->m_pmcSelectedMesh->ptmDrawMesh->GetNumFaces());
            }
    ...
    )
{
    ...
    if (pmc->pPMMesh != NULL)
    {
        StringCchPrintfA(szOutput,szOutputLen, "Information about selected progressive mesh:\r\n");

        StringCchPrintfA(szBuf, 120, "\tMin number of faces   : %d\r\n", pmc->pPMMesh->GetMinFaces());
        StringCchCatA(szOutput, szOutputLen, szBuf);
        StringCchPrintfA(szBuf, 120, "\tCur number of faces   : %d\r\n", cFaces);
        StringCchCatA(szOutput, szOutputLen, szBuf);
        StringCchPrintfA(szBuf, 120, "\tMax number of faces   : %d\r\n\r\n", pmc->pPMMesh->GetMaxFaces());
        StringCchCatA(szOutput, szOutputLen, szBuf);

        StringCchPrintfA(szBuf, 120, "\tMin number of vertices: %d\r\n", pmc->pPMMesh->GetMinVertices());
        StringCchCatA(szOutput, szOutputLen, szBuf);
        StringCchPrintfA(szBuf, 120, "\tCur number of vertices: %d\r\n", cVertices);
        StringCchCatA(szOutput, szOutputLen, szBuf);
        StringCchPrintfA(szBuf, 120, "\tMax number of vertices: %d\r\n", pmc->pPMMesh->GetMaxVertices());
        StringCchCatA(szOutput, szOutputLen, szBuf);
    }
    else
    {
        StringCchPrintfA(szOutput, szOutputLen, "Information about selected mesh:\r\n");

        StringCchPrintfA(szBuf, 120, "\tNumber of faces   : %d\r\n", cFaces);
        StringCchCatA(szOutput, szOutputLen, szBuf);
        StringCchPrintfA(szBuf, 120, "\tNumber of vertices: %d\r\n", cVertices);
        StringCchCatA(szOutput, szOutputLen, szBuf);
    }
