...
DlgProcSplitMesh(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        StringCchPrintfA(szBuf, 1024, "%d", g_pData->m_pmcSelectedMesh->ptmDrawMesh->GetNumFaces());
        SetDlgItemText(hDlg, IDC_SPLITMESH_FACES, szBuf);

        StringCchPrintfA(szBuf, 1024, "%d", g_pData->m_pmcSelectedMesh->ptmDrawMesh->GetNumVertices());
        SetDlgItemText(hDlg, IDC_SPLITMESH_VERTICES, szBuf);

        StringCchPrintfA(szBuf, 1024, "%d", iSplitSize);
...
DlgProcSimplifyInputs(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Position);
        SetDlgItemText(hDlg, IDC_POSITIONWEIGHT, szBuf);

        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Boundary);
        SetDlgItemText(hDlg, IDC_BOUNDARYWEIGHT, szBuf);

        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Normal);
        SetDlgItemText(hDlg, IDC_NORMALWEIGHT, szBuf);

        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Diffuse);
        SetDlgItemText(hDlg, IDC_DIFFUSEWEIGHT, szBuf);

        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Specular);
        SetDlgItemText(hDlg, IDC_SPECULARWEIGHT, szBuf);

        StringCchPrintfA(szBuf, 1024, "%f", pAttributeWeights->Texcoord[0]);
...
TrivialData::RemoveBackToBackTris()
{
    ...
    if ((m_pmcSelectedMesh != NULL) && (m_pmcSelectedMesh->pMesh != NULL))
    {
        ...
        StringCchPrintf(szBuf, 80,"Removed %d back to back tris\n", cFacesRemoved);
...
SimulateCache(DWORD iVertexCacheSize, LPD3DXMESH pMesh, BOOL bLRU, char szBuf[80], UINT_IDX Dummy)
{
    ...
    if (bLRU)
        StringCchPrintfA(szBuf, 80, "LRU  Cache Size: %d, Misses: %d, Miss Rate: %f\r\n", iVertexCacheSize, cMisses, (float)cMisses / (float)pMesh->GetNumVertices());
    else
        StringCchPrintfA(szBuf, 80, "FIFO Cache Size: %d, Misses: %d, Miss Rate: %f\r\n", iVertexCacheSize, cMisses, (float)cMisses / (float)pMesh->GetNumVertices());
...
GenerateCacheInfo(LPD3DXMESH pMesh, char **pszBuffer)
{
	...
	UINT szOutputLen= 120 * (x_cCacheSizes*2 + 2);
    ...
    StringCchPrintfA(szOutput,szOutputLen, "Miss rates for using optimized mesh with FIFO caches (Mesh optimized for %d entries)\r\n",12);
...
TrivialData::SimulateCacheStuff()
{
	...
	UINT szOutputLen= 120 * ((x_cOptCacheSizes+3)*x_cHWCacheSizes*2);
    szOutput = new char[szOutputLen];
    if (szOutput == NULL)
    {
        hr = E_OUTOFMEMORY;
    ...
    for (iOptVertexCacheSize = x_iOptMaxCacheSize; iOptVertexCacheSize >= x_iOptMinCacheSize; iOptVertexCacheSize--)
    {
        ...
        StringCchPrintfA(szBuf, 120, "%d", iOptVertexCacheSize);
        //sprintf(szBuf, "%d", 3);
        ...
        StringCchPrintfA(szBuf, 120, "Optimized for vertex cache of %d entries\r\n", iOptVertexCacheSize);
        StringCchCatA(szOutput, szOutputLen, szBuf);
...
DlgProcWeldVertices(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
        ...
        StringCchPrintfA(szBuf, 1024, "%f", 0.01f);
...
DlgProcTestSimplify(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        {
        case IDOK:
            GetDlgItemText(hDlg, IDC_TESTINTERVAL, szBuf, sizeof(szBuf));
