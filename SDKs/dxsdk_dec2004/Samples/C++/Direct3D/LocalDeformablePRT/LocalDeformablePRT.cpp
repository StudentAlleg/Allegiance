...
void InitApp()
{
    ...
    g_HUD.AddButton( IDC_CHANGEDEVICE, L"Change device (F2)",     iX, iY += 24, 125, 22 );
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    V_RETURN( g_LightControl.StaticOnCreateDevice( pd3dDevice ) );
...
HRESULT LoadTechniqueObjects( const char* szMedia )
{
    ...
    IDirect3DDevice9 *pDevice = DXUTGetD3DDevice();
    
    WCHAR strFileName[MAX_PATH+1] = {0};
    WCHAR strPath[MAX_PATH+1] = {0};
    
    // Make sure the technique works
    char* strTechnique = (char*)g_SampleUI.GetComboBox(IDC_TECHNIQUE)->GetSelectedData();
    ...
    bool bLDPRT = ( 0 == strcmp( strTechnique, "LDPRT" ) );
...
void CALLBACK OnFrameRender( IDirect3DDevice9* pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        V( g_LightControl.OnRender( D3DXCOLOR(1, 1, 1, 1), (D3DXMATRIX*)g_Camera.GetViewMatrix(), (D3DXMATRIX*)g_Camera.GetProjMatrix(), g_Camera.GetEyePt() ) );
        DXUT_EndPerfEvent();
...
void RenderText()
{
    ...
    if( g_bShowHelp )
    {
        ...
        txtHelper.DrawTextLine( L"Rotate Camera: Right Mouse" );
