...
HRESULT CSHFunctionViewer::OnCreateDevice( LPDIRECT3DDEVICE9 pd3dDevice )
{
    ...
    for(fi=0;fi<m_uNumPhi;fi++) 
    {
        pIBuff[fi*3 + 0] = 0;
        pIBuff[fi*3 + 1] = fi+1;
        pIBuff[fi*3 + 2] = ((fi+1)%m_uNumPhi)+1;
        rfi++;
    }
    ...
    for(vi=1;vi<m_uNumTheta-2;vi++) 
    {
        for(fi=0;fi<m_uNumPhi;fi++) 
        {
            pIBuff[rfi*3 + 0] = 1 + (vi-1)*m_uNumPhi + fi;
            pIBuff[rfi*3 + 1] = 1 + (vi)*m_uNumPhi + fi;
            pIBuff[rfi*3 + 2] = 1 + (vi)*m_uNumPhi + ((fi+1)%m_uNumPhi);

            rfi++;

            pIBuff[rfi*3 + 0] = 1 + (vi-1)*m_uNumPhi + fi;
            pIBuff[rfi*3 + 1] = 1 + (vi)*m_uNumPhi + ((fi+1)%m_uNumPhi);
            pIBuff[rfi*3 + 2] = 1 + (vi-1)*m_uNumPhi + ((fi+1)%m_uNumPhi);
            rfi++;
        }
    }
    ...
    for(fi=0;fi<m_uNumPhi;fi++) 
    {
        pIBuff[rfi*3 + 0] = 1 + m_uNumPhi*(m_uNumTheta-3) + fi;
        pIBuff[rfi*3 + 1] = 1 + m_uNumPhi*(m_uNumTheta-2);
        pIBuff[rfi*3 + 2] = 1 + m_uNumPhi*(m_uNumTheta-3) + ((fi+1)%m_uNumPhi);
        rfi++;
    }
