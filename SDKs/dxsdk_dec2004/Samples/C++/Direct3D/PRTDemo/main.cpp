...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    g_LightProbe[g_dwLightProbeA].OnCreateDevice( DXUTGetD3DDevice(), g_LightProbeFiles[g_dwLightProbeA], true );
    g_LightProbe[g_dwLightProbeB].OnCreateDevice( DXUTGetD3DDevice(), g_LightProbeFiles[g_dwLightProbeB], true );
...
void CALLBACK OnGUIEvent( UINT nEvent, int nControlID, CDXUTControl* pControl )
{
    switch( nControlID )
    {
        ...
        case IDC_ENVIRONMENT_A:
            ...
            if( !g_LightProbe[g_dwLightProbeA].IsCreated() && DXUTGetD3DDevice() )
            {
                g_LightProbe[g_dwLightProbeA].OnCreateDevice( DXUTGetD3DDevice(), g_LightProbeFiles[g_dwLightProbeA], true );
                g_LightProbe[g_dwLightProbeA].OnResetDevice( DXUTGetBackBufferSurfaceDesc() ); 
            }
            break;

        case IDC_ENVIRONMENT_B:
            g_dwLightProbeB = PtrToInt( g_RenderingUI.GetComboBox( IDC_ENVIRONMENT_B )->GetSelectedData() );
            if( !g_LightProbe[g_dwLightProbeB].IsCreated() && DXUTGetD3DDevice() )
            {
                g_LightProbe[g_dwLightProbeB].OnCreateDevice( DXUTGetD3DDevice(), g_LightProbeFiles[g_dwLightProbeB], true );
                g_LightProbe[g_dwLightProbeB].OnResetDevice( DXUTGetBackBufferSurfaceDesc() ); 
            }
            break;
        ...
        case IDC_APPLY:
        {
            ...
            g_Simulator.CompressInWorkerThread( DXUTGetD3DDevice(), &options, &g_PRTMesh );
            g_AppState = APP_STATE_SIMULATOR_RUNNING;
            break;
        }
    }
