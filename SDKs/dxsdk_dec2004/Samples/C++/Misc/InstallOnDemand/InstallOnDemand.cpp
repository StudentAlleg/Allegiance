...
CDXUTMesh               g_Speeder;              // Mesh for the speeder
CDXUTMesh               g_IntroTerrain;         // Mesh for the intro terrain
CDXUTMesh               g_LevelMesh;            // Mesh for level terrain
...
HRESULT CALLBACK OnCreateDevice( IDirect3DDevice9* pd3dDevice, const D3DSURFACE_DESC* pBackBufferSurfaceDesc )
{
    ...
    V_RETURN( g_LevelMesh.SetFVF( pd3dDevice, g_LevelMesh.GetSysMemMesh()->GetFVF() | D3DFVF_NORMAL ) );
    ...
    V_RETURN( g_Speeder.SetFVF( pd3dDevice, g_LevelMesh.GetSysMemMesh()->GetFVF() | D3DFVF_NORMAL ) );
    ...
    V_RETURN( g_IntroTerrain.SetFVF( pd3dDevice, g_LevelMesh.GetSysMemMesh()->GetFVF() | D3DFVF_NORMAL ) );
...
void RenderLoadingScreenBackground( IDirect3DDevice9* pd3dDevice )
{
    ...
    const D3DSURFACE_DESC *pdsd = DXUTGetBackBufferSurfaceDesc();
    QUADVERT quad[4] =
    {
...
void RenderIntro( IDirect3DDevice9 *pd3dDevice, double fTime, float fElapsedTime )
{
    ...
    V( pd3dDevice->SetFVF( g_Speeder.GetLocalMesh()->GetFVF() ) );
    ...
    m *= mProj;
    V( g_pEffect->SetMatrix( "g_mWorldViewProjection", &m ) );
    V( pd3dDevice->SetFVF( g_IntroTerrain.GetLocalMesh()->GetFVF() ) );
    ...
    if( SUCCEEDED( pd3dDevice->BeginScene() ) )
    {
        ...
        if( AppState_Normal == g_AppState )
        {
            if( -1 != g_nCurrentLevel )
            {
                ...
                pd3dDevice->SetFVF( g_LevelMesh.GetLocalMesh()->GetFVF() );
    ...
    switch( nControlID )
    {
        ...
        case IDC_LOADLEVEL1:
            if( g_nCurrentLevel != 0 )
            {
                ...
                LoadLevel( DXUTGetD3DDevice(), false );
        ...
        case IDC_NEW_ACCEPT:
            ...
            g_NewGameMenu.SetVisible( false );
            g_AppState = AppState_Normal;
            LoadLevel( DXUTGetD3DDevice(), true );
            break;
